...
class Compiler {
    constructor(options = {}) {
        var _a, _b, _c, _d, _e;
        this.options = {
            minify: (_a = options.minify) !== null && _a !== void 0 ? _a : false,
            dedupe: (_b = options.dedupe) !== null && _b !== void 0 ? _b : true,
            sourceMap: (_c = options.sourceMap) !== null && _c !== void 0 ? _c : false,
            inlineSourceMap: (_d = options.inlineSourceMap) !== null && _d !== void 0 ? _d : false,
            assets: (_e = options.assets) !== null && _e !== void 0 ? _e : true,
    ...
    compileEntry(entryPath) {
        ...
        const entries = (0, helpers_1.discoverEntries)(entryPath);
    ...
    compileString(source, filePath = `inline${constants_1.PRIMARY_STYLE_EXTENSION}`) {
        this.reset();
        const stylesheet = new parser_1.Parser(source, filePath).parse();
    ...
    loadStylesheet(filePath) {
        ...
        const stylesheet = new parser_1.Parser(source, absolutePath).parse();
    ...
    loadBuiltinStylesheet(moduleId) {
        const cached = this.parsedFiles.get(moduleId);
        if (cached) {
            return cached;
        }
        const moduleDefinition = (0, builtin_modules_1.resolveBuiltinModule)(moduleId);
        if (!moduleDefinition) {
            throw new Error(`Unknown built-in module "${moduleId}"`);
        }
        const stylesheet = new parser_1.Parser(moduleDefinition.source, moduleDefinition.id).parse();
    ...
    resolveImportReference(importPath, fromFile) {
        const builtinModule = (0, builtin_modules_1.resolveBuiltinModule)(importPath);
        if (builtinModule) {
            return {
                id: builtinModule.id,
        ...
        const resolvedPath = (0, helpers_1.resolveImportPath)(importPath, fromFile);
    ...
    processImportedStylesheet(importReference, environment, currentSelectors, atRules) {
    ...
    processStatements(statements, environment, currentSelectors, atRules) {
        const declarations = [];
        ...
        for (const statement of statements) {
            switch (statement.type) {
                ...
                case 'Declaration': {
                    const compiled = this.compileDeclaration(statement.property, statement.value, environment.scope, statement.source, currentSelectors);
    ...
    registerContextVariables(atRule, environment, currentSelectors) {
        if (!atRule.body) {
            return;
        }
    ...
    expandInclude(include, environment, currentSelectors, atRules) {
        ...
        mixin.params.forEach((parameter, index) => {
            var _a;
            const value = (_a = include.args[index]) !== null && _a !== void 0 ? _a : parameter.defaultValue;
            if (value !== undefined) {
    ...
    expandEach(eachNode, environment, currentSelectors, atRules) {
        const combined = {
            declarations: [],
            darkDeclarations: [],
            pendingLocks: []
        ...
        eachNode.items
            ...
            .forEach(item => {
            expandedItems.push(...(0, helpers_1.splitByComma)(item));
        });
    ...
    expandFor(forNode, environment, currentSelectors, atRules) {
    ...
    compileRule(rule, environment, currentSelectors, atRules) {
    ...
    compileAtRule(atRule, environment, currentSelectors, atRules) {
    ...
    compileDeclaration(property, rawValue, scope, source, currentSelectors) {
    ...
    emitRule(selectors, declarations, atRules, source) {
        ...
        for (const selector of ruleSelectors) {
            for (const declaration of ruleDeclarations) {
                ...
                if (lockedValue && lockedValue !== declaration.value) {
                    throw new Error(`${constants_1.PRIMARY_LOCK_NAME} blocked override of ${selector} { ${declaration.property}: ${lockedValue} } at ${this.formatLocation(source)}`);
                }
            }
        }
        this.rules.push({
    ...
    registerLocks(selectors, declarations, locks, atRules, scope, source) {
        if (locks.length === 0) {
            return;
        }
        for (const lock of locks) {
            ...
            for (const selector of lockSelectors) {
                for (const property of lock.properties) {
                    ...
                    if (!declaration) {
                        throw new Error(`Unable to lock "${resolvedProperty}" for ${selector}; property not found at ${this.formatLocation(source)}`);
                    }
    ...
    extractSharedUtilities(rules) {
        const groupedRuleIndexes = new Map();
        ...
        groupedRuleIndexes.forEach(indexes => {
            ...
            const usedSignaturesByRule = new Map();
            ...
            candidates.forEach(candidate => {
                ...
                const utilitySelector = `.${constants_1.PRIMARY_UTILITY_PREFIX}-${utilityId}`;
                ...
                candidate.signatures.forEach((signature, index) => {
                    ...
                    validRuleIndexes.forEach(ruleIndex => {
                        const replacementMap = replacements.get(ruleIndex) || new Map();
                        replacementMap.set(signature, `var(${variableName})`);
                        replacements.set(ruleIndex, replacementMap);
                    });
                });
                validRuleIndexes.forEach(ruleIndex => {
                    const used = usedSignaturesByRule.get(ruleIndex) || new Set();
    ...
    collectUsage(contentPaths) {
        ...
        for (const contentPath of contentPaths) {
            ...
            if (stats.isDirectory()) {
                files.push(...(0, helpers_1.walkDirectory)(absolutePath, '.html', true));
                files.push(...(0, helpers_1.walkDirectory)(absolutePath, '.js', true));
                files.push(...(0, helpers_1.walkDirectory)(absolutePath, '.jsx', true));
                files.push(...(0, helpers_1.walkDirectory)(absolutePath, '.ts', true));
                files.push(...(0, helpers_1.walkDirectory)(absolutePath, '.tsx', true));
            }
            else {
        ...
        for (const filePath of this.unique(files)) {
            ...
            this.collectRegexMatches(content, /class(?:Name)?\s*=\s*["'`]([^"'`]+)["'`]/g).forEach(match => {
                match[1].split(/\s+/).filter(Boolean).forEach((name) => usage.classes.add(name));
            });
            ...
            this.collectRegexMatches(content, /classList\.(?:add|remove|toggle)\(([^)]+)\)/g).forEach(match => {
                match[1]
                    ...
                    .filter(Boolean)
                    .forEach((name) => usage.classes.add(name));
            });
    ...
    collectSelectorTokens(selector, usage) {
    ...
    render(rules, rawOutputs) {
        const compact = this.options.minify;
        const mapBuilder = (this.options.sourceMap || this.options.inlineSourceMap)
            ? new source_map_1.SourceMapBuilder(this.sourceContents, this.options.outputFile)
            : undefined;
        let css = '';
        const append = (text, source) => {
    ...
    convertAssetUrl(rawUrl, source) {
        ...
        if (!this.isAssetCurrent(absoluteInputPath, outputAssetPath)) {
            ...
            if (!converted) {
                this.warn(`Unable to convert ${absoluteInputPath} to WebP. Install cwebp, ImageMagick, ffmpeg, or set ${constants_1.PRIMARY_WEBP_ENV_VAR}.`);
                return null;
            }
        }
    ...
    resolveAssetOutputDirectory(outputFile) {
        ...
        return path.join(path.dirname(outputFile), constants_1.PRIMARY_ASSET_DIR);
    }
    buildAssetFileName(inputPath) {
        const parsed = path.parse(inputPath);
        const hash = crypto.createHash('sha1').update(inputPath).digest('hex').slice(0, 8);
        return `${parsed.name}-${hash}.webp`;
    }
    isAssetCurrent(inputPath, outputPath) {
        if (!fs.existsSync(outputPath)) {
            return false;
        }
        return fs.statSync(outputPath).mtimeMs >= fs.statSync(inputPath).mtimeMs;
    }
    convertAssetToWebp(inputPath, outputPath) {
        const customConverter = process.env[constants_1.PRIMARY_WEBP_ENV_VAR] || process.env[constants_1.LEGACY_WEBP_ENV_VAR];
    ...
    runAssetConverter(command, args) {
    ...
    combineSelectors(parentSelectors, selectorText) {
    ...
    splitSelectors(selectorText) {
        return (0, helpers_1.splitByComma)(selectorText).map(selector => selector.trim()).filter(Boolean);
    }
    resolveSelectorText(text, scope, currentSelectors = []) {
        return this.interpolate(text, scope, currentSelectors).replace(/\s+/g, ' ').trim();
    }
    resolveValue(rawValue, scope, currentSelectors = [], useContextVariables = true) {
    ...
    interpolate(input, scope, currentSelectors = [], useContextVariables = true) {
        let value = input;
        value = value.replace(/#\{\s*(\$[\w-]+)\s*\}/g, (_match, variable) => { var _a; return (_a = this.lookupVariable(scope, variable, currentSelectors, useContextVariables)) !== null && _a !== void 0 ? _a : variable; });
        value = value.replace(/\$[\w-]+/g, (variable) => { var _a; return (_a = this.lookupVariable(scope, variable, currentSelectors, useContextVariables)) !== null && _a !== void 0 ? _a : variable; });
        return value;
    }
    lookupVariable(scope, name, currentSelectors = [], useContextVariables = true) {
        let current = scope;
        let rootValue;
        while (current) {
    ...
    lookupContextVariable(name, currentSelectors) {
        let bestValue;
        let bestScore = -1;
    ...
    contextMatchesSelectors(contextSelector, currentSelectors) {
        return currentSelectors.some(currentSelector => this.selectorContainsContext(currentSelector, contextSelector));
    }
    selectorContainsContext(currentSelector, contextSelector) {
    ...
    expandPrefixes(property, value, source) {
        const declarations = [];
    ...
    applyFunction(value, functionName, handler) {
        let output = value;
        let cursor = 0;
        while (cursor < output.length) {
            ...
            const args = (0, helpers_1.splitByComma)(output.slice(openParen + 1, closeParen));
    ...
    findFunctionIndex(value, functionName, fromIndex) {
    ...
    findMatchingParen(value, openParenIndex) {
        let depth = 0;
        let quote = '';
    ...
    containerUnit(rawValue, unit, functionName) {
        if (!rawValue) {
            return `100${unit}`;
        }
        ...
        const normalized = value.replace(/(-?\d*\.?\d+)%/g, (_match, numeric) => `${numeric}${unit}`);
        if (normalized === value) {
    ...
    mixColors(left, right, weight) {
        ...
        const blend = (a, b) => Math.round(a * ratio + b * (1 - ratio));
        return this.formatColor({
        ...
        if (rgb) {
            ...
            if (values.length >= 3) {
                return {
                    ...
                    g: values[1],
                    b: values[2],
                    a: (_a = values[3]) !== null && _a !== void 0 ? _a : 1
                };
            }
        }
        return null;
    ...
    evaluateCondition(condition, scope, currentSelectors = []) {
    ...
    createLockKey(selector, atRules, property) {
        return `${selector}::${atRules.join('|')}::${property}`;
    }
    collapseDeclarations(declarations) {
        const output = [];
        const seen = new Set();
    ...
    utilityVariableName(utilityId, property, index) {
        const normalizedProperty = property.replace(/[^a-zA-Z0-9-]/g, '-');
        return `--${constants_1.PRIMARY_UTILITY_PREFIX}-${utilityId}-${normalizedProperty}-${index}`;
    }
    unique(values) {
        return Array.from(new Set(values));
    }
    collectRegexMatches(input, expression) {
    ...
    sameAtRules(left, right) {
        return left.length === right.length && left.every((value, index) => value === right[index]);
    }
    leftPad(value, length, padCharacter) {
        let output = value;
